...
Output:
    - Binary:    {platform}_{version}_b{build_number}.bin
...
def update_version_in_ini(version):
    ...
    pattern = re.compile(r'^(\s*version\s*=\s*)(\S+)(\s*;.*)?$', re.MULTILINE)
...
def build_firmware(env_name, version):
    """Run generate_buildinfo.py and PlatformIO build."""
    run([
        sys.executable,
...
def create_release(env_name, version, dry_run=False, skip_push=False, skip_gh_release=False):
    """Copy binary, create git tag, create GitHub Release with binary asset."""
    ...
    _, _, build_number, config = read_config()
    ...
    # Step 1: Copy binary to project root
    print(f"\n\033[1;33m[1/5] Copying binary\033[0m")
    print(f"  {firmware_bin} -> {bin_dest}")
    shutil.copy2(firmware_bin, bin_dest)

    # Step 2: Stage and commit
    print(f"\n\033[1;33m[2/5] Committing\033[0m")
    run(["git", "add", str(bin_dest), str(PLATFORMIO_INI)])
    commit_msg = f"release: {tag}"
    run(["git", "commit", "-m", commit_msg])

    # Step 3: Create annotated tag
    print(f"\n\033[1;33m[3/5] Creating tag {tag}\033[0m")
    run(["git", "tag", "-a", tag, "-m", f"Release {tag}"])

    # Step 4: Push to remote
    if not skip_push:
        print(f"\n\033[1;33m[4/5] Pushing to origin\033[0m")
        run(["git", "push", "origin", "HEAD"])
        run(["git", "push", "origin", tag])
    else:
        print(f"\n\033[1;33m[4/5] Skipping push (--skip-push)\033[0m")

    # Step 5: Create GitHub Release
    if not skip_gh_release:
        print(f"\n\033[1;33m[5/5] Creating GitHub Release\033[0m")

        gh_ok = check_gh_cli()
        if not gh_ok:
            print("\033[1;31mCannot create GitHub Release without authenticated gh CLI.\033[0m")
            print(f"After authenticating, run manually:")
            print(f"  gh release create {tag} '{bin_dest}' --title 'v{version}-b{build_number}' --notes-from-tag")
            return
...
def main():
    ...
    parser.add_argument(
        "--skip-push",
        action="store_true",
        help="Don't push tag/commit to remote",
    )
    parser.add_argument(
        "--skip-gh-release",
        action="store_true",
        help="Only create tag + binary, skip GitHub Release creation",
    )
    ...
    print(f"Current build number: {build_number} (will auto-increment)")

    if args.dry_run:
        platform_lower = get_platform_string_from_config(config, args.env)
        tag = f"v{version}-b{build_number + 1}"
        bin_name = f"{platform_lower}_{version}_b{build_number + 1}.bin"
        print(f"\n\033[1;33m[DRY RUN]\033[0m")
        print(f"  Would build env:    {args.env}")
        print(f"  Would create tag:   {tag}")
        print(f"  Would create binary: {bin_name}")
        if not args.skip_gh_release:
            print(f"  Would create GitHub Release with {bin_name} as asset")
        if args.version:
            print(f"  Would update platformio.ini version to: {version}")
        return

    # Persist version override
    if args.version:
