...
def create_release(env_name, version, dry_run=False, skip_push=False, skip_gh_release=False):
    ...
    # Step 1: Create annotated tag (pointing to current HEAD, no commit needed)
    print(f"\n\033[1;33m[1/3] Creating tag {tag}\033[0m")
    run(["git", "tag", "-a", tag, "-m", f"Release {tag}"])

    # Step 2: Push tag to remote
    if not skip_push:
        print(f"\n\033[1;33m[2/3] Pushing tag to origin\033[0m")
        run(["git", "push", "origin", tag])
    else:
        print(f"\n\033[1;33m[2/3] Skipping push (--skip-push)\033[0m")

    # Step 3: Create GitHub Release (binary uploaded directly from build dir)
    if not skip_gh_release:
        print(f"\n\033[1;33m[3/3] Creating GitHub Release\033[0m")

        gh_ok = check_gh_cli()
        if not gh_ok:
            print("\033[1;31mCannot create GitHub Release without authenticated gh CLI.\033[0m")
            print(f"After authenticating, run manually:")
            print(f"  gh release create {tag} '{firmware_bin}#{bin_name}' --title 'v{version}-b{build_number}' --notes-from-tag")
            return
