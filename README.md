...
В датчиках семейства INA ток измеряется косвенно: на токовом шунте известного сопротивления измеряется падение напряжения, и ток вычисляется по закону Ома:
...
1. **Ток в цепи** — един для всей последовательной цепи (шунт + нагрузка):
   ...
   $$
   I = \frac{U_{shunt}}{R_{shunt}} = \frac{U_{\text{нагр}}}{R_{\text{нагр}}}
   $$

2. **Напряжение источника питания** равно сумме падений напряжений на шунте и нагрузке:

   $$
   U_{\text{ист}} = U_{shunt} + U_{\text{нагр}}
   $$

   Поскольку сопротивление шунта значительно меньше сопротивления нагрузки:
   
   $$
   R_{shunt} \ll R_{\text{нагр}}
   $$
   
   (шунт имеет сопротивление единицы-десятки мОм, а нагрузка — единицы-сотни Ом), падением напряжения на шунте можно пренебречь:
   ...
   $$
   U_{\text{ист}} \approx U_{\text{нагр}}
   $$
   
   Именно это напряжение измеряет датчик INA между выводами GND и IN-.

3. **Мгновенная мощность**, потребляемая нагрузкой:

   $$
   P = U_{\text{нагр}} \times I
   $$
...
5. **Потреблённая энергия** (интеграл мощности по времени):
   ...
   $$
   E = \int_{t_0}^{t_1} U_{\text{нагр}}(t) \cdot I(t) \, dt \quad \text{[Дж]}
   $$

   $$
   E_{mWh} = \frac{1000}{3600} \int_{t_0}^{t_1} U_{\text{нагр}}(t) \cdot I(t) \, dt \quad \text{[mWh]}
   $$
...
In the INA family of sensors, current is measured indirectly: the voltage drop is measured across a current shunt of known resistance, and the current is calculated using Ohm's law:
...
2. **Source voltage** equals the sum of voltage drops across the shunt and the load:
   ...
   Since the shunt resistance is much smaller than the load resistance:
   
   $$
   R_{shunt} \ll R_{load}
   $$
   
   (the shunt has a resistance of units to tens of mΩ, while the load has units to hundreds of Ω), the voltage drop across the shunt can be neglected:
   ...
   $$
   U_{src} \approx U_{load}
   $$
   
   The INA sensor measures exactly this voltage between the GND and IN- pins.
